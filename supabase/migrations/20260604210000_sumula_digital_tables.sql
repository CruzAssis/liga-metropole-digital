...
DROP POLICY IF EXISTS "Public read match_best_own_votes" ON public.match_best_own_votes;
CREATE POLICY "Public read match_best_own_votes"
ON public.match_best_own_votes FOR SELECT USING (true);

DROP POLICY IF EXISTS "Authenticated write match_best_own_votes" ON public.match_best_own_votes;
...
DROP POLICY IF EXISTS "Public read match_destaques_publicados" ON public.match_destaques_publicados;
CREATE POLICY "Public read match_destaques_publicados"
ON public.match_destaques_publicados FOR SELECT USING (true);

DROP POLICY IF EXISTS "Authenticated write match_destaques_publicados" ON public.match_destaques_publicados;
