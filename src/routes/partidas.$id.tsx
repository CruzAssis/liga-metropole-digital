        ...
        {isParticipante && (
          <>
            <div className="bg-zinc-900 rounded-xl p-6 border border-zinc-800 space-y-4">
              ...
              <p className="text-xs text-zinc-500">{isVisitante ? 'Como Visitante, voce lanca o placar primeiro. O Mandante confirma.' : 'Confirme o placar lancado pelo Visitante.'}</p>
              <div className="grid grid-cols-3 items-center gap-4">
                <div>
            ...
            <div className="bg-zinc-900 rounded-xl p-6 border border-zinc-800 space-y-4">
              ...
              <p className="text-xs text-zinc-500">Registre os gols marcados pelo seu time nesta partida.</p>
              {gols.map((gol, idx) => (
                <div key={idx} className="grid grid-cols-2 gap-3 p-3 bg-zinc-800 rounded-lg">
                  ...
                  {gols.length > 1 && <button type="button" onClick={() => setGols(prev => prev.filter((_, i) => i !== idx))} className="col-span-2 text-xs text-red-400 text-right hover:text-red-300">Remover gol</button>}
                </div>
              ))}
              <Button type="button" variant="outline" onClick={() => setGols(prev => [...prev, { jogador_nome: '', assistencia_nome: '' }])} className="w-full border-zinc-700 text-zinc-400 hover:border-[#1565F5] hover:text-[#1565F5]">
                + Adicionar gol
              </Button>
            </div>
            ...
            <div className="bg-zinc-900 rounded-xl p-6 border border-zinc-800 space-y-4">
              ...
              {destaques.map((d, idx) => (
                <div key={idx} className="grid grid-cols-2 gap-3 p-3 bg-zinc-800 rounded-lg">
                  ...
                  <div>
                    <Label className="text-zinc-400 text-xs">Num da camisa no jogo</Label>
                    <Input value={d.numero_camisa} onChange={e => setDestaques(prev => prev.map((x, i) => i === idx ? { ...x, numero_camisa: e.target.value } : x))} className="mt-1 bg-zinc-700 border-zinc-600 text-white text-sm" placeholder="Ex: 10" />
                  </div>
                </div>
              ))}
              <div className="border-t border-zinc-700 pt-4 space-y-3">
                <p className="text-sm text-zinc-300">Candidatos do adversario - escolha o destaque e de uma nota (0-10):</p>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label className="text-zinc-400 text-xs">Nota do jogador (0-10)</Label>
                    <Input type="number" min="0" max="10" step="0.1" value={notaDestaque} onChange={e => setNotaDestaque(e.target.value)} className="mt-1 bg-zinc-800 border-zinc-700 text-white" placeholder="Ex: 8.5" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-zinc-900 rounded-xl p-6 border border-zinc-800 space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <Label htmlFor="incidente" className="text-zinc-300 cursor-pointer">Houve atitude inadequada ou incorreta da arbitragem</Label>
              </div>
              {houveIncidente && (
                <div>
                  <Label className="text-zinc-400 text-xs">Descreva o ocorrido</Label>
                  <Textarea value={descricaoIncidente} onChange={e => setDescricaoIncidente(e.target.value)} className="mt-1 bg-zinc-800 border-zinc-700 text-white min-h-[80px]" placeholder="Descreva o que aconteceu com a arbitragem..." />
                </div>
              )}
            </div>
