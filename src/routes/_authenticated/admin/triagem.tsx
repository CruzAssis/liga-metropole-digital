...
function TriagemPage() {
  ...
  async function aprovarTime(teamId) {
    ...
    setSaving(teamId)
    try {
      const { error } = await supabase.from('teams').update({ lado: config.lado, grupo: config.grupo, serie: config.serie, mensalidade_paga: true }).eq('id', teamId)
      if (error) throw error
      toast({ title: 'Time aprovado com sucesso!' })
      setTeams(prev => prev.filter(t => t.id !== teamId))
    } catch (err) {
      toast({ title: err.message || 'Erro ao aprovar time', variant: 'destructive' })
    } finally {
      setSaving(null)
    }
  }
  ...
  if (loading) return <div className="p-8 text-zinc-400">Carregando inscricoes...</div>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <p className="text-zinc-500 text-xs uppercase tracking-wide">Total pendente</p>
          <p className="text-3xl font-bold text-[#1565F5] mt-1">{teams.length}</p>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <p className="text-zinc-500 text-xs uppercase tracking-wide">Meta de vagas</p>
      ...
      {teams.length === 0 && (
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-12 text-center">
          <p className="text-zinc-400">Nenhuma inscricao pendente no momento.</p>
        </div>
      )}
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <Badge variant="outline" className={team.tipo === 'mandante' ? 'border-blue-600 text-blue-400 text-xs' : 'border-purple-600 text-purple-400 text-xs'}>
                        {team.tipo === 'mandante' ? 'Mandante' : 'Visitante'}
                      </Badge>
